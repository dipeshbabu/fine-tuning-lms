...
# Quiet tokenizers fork warnings if you want
...
class StopOnSemicolon(StoppingCriteria):
    def __init__(self, tokenizer):
        ...
        # SentencePiece has a literal ";" token id
...
def train_epoch(args, model, train_loader, optimizer, scheduler):
    ...
    # Modern AMP API to silence deprecation warnings
...
def _make_gen(tok: T5TokenizerFast, args) -> GenerationConfig:
    # No constrained decoding here (keeps compatibility with older transformers)
...
def _generate_sql_batches(model, dev_loader, gen_cfg, tok, stoppers):
    sql_preds = []
    with torch.no_grad():
        for batch in tqdm(dev_loader, desc="Generate"):
            ...
            # Try beam; if the installed transformers lacks something, fall back to greedy
            try:
                out = model.generate(
                    input_ids=encoder_input,
                    attention_mask=encoder_mask,
...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    gen_cfg = _make_gen(tok, args)

    sql_preds = _generate_sql_batches(model, dev_loader, gen_cfg, tok, stoppers)
    ...
    # Surface a few DB errors for debugging
    if model_error_msgs:
...
def test_inference(args, model, test_loader, model_sql_path, model_record_path):
    ...
    sql_preds = _generate_sql_batches(model, test_loader, gen_cfg, tok, stoppers)
...
def main():
    ...
    model_type = "ft" if args.finetune else "scr"
    gt_sql_path = os.path.join(DATA_DIR, "dev.sql")
    gt_record_path = os.path.join(RECORDS_DIR, "ground_truth_dev.pkl")  # must exist already
    model_sql_path = os.path.join(RESULTS_DIR, f"t5_{model_type}_{experiment_name}.sql")
    model_record_path = os.path.join(RECORDS_DIR, f"t5_{model_type}_{experiment_name}.pkl")
